...
# The routine fires once per weekday at 13:00 London time.
#
# London is normally 5 hours ahead of New York, so 13:00 London = 08:00 ET.
# For roughly four weeks a year the two are only 4 hours apart -- the US
# changes clocks in mid-March and reverts in early November, the UK in late
# March and late October -- and during those windows 13:00 London = 09:00 ET.
#
# Both land before the 09:30 open, which is what matters. Any other hour means
# the trigger has been changed or the run fired far off schedule, and is
# rejected rather than producing a brief at the wrong time of day.
PRE_OPEN_HOURS = (8, 9)
...
def should_run(now: datetime | None = None) -> tuple[bool, str]:
    """True only on a trading day, during the pre-open window."""
    now = (now or datetime.now(NY)).astimezone(NY)
    if not is_trading_day(now.date()):
        return False, f"NYSE closed on {now.date()}"
    if now.hour not in PRE_OPEN_HOURS:
        return False, (
            f"outside the pre-open window (ET hour {now.hour:02d}, "
            f"expected {' or '.join(f'{h:02d}' for h in PRE_OPEN_HOURS)})"
        )
    mins = (9 - now.hour) * 60 + (30 - now.minute)
    return True, f"trading day, {now:%Y-%m-%d %H:%M %Z}, {mins} min to the open"
